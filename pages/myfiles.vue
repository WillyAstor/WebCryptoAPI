...
export default {
  head() {
    return {
      title: `2file - My Previous Uploads`,
    }
  },
  data() {
    return {
  ...
  mounted() {
    let uploads = localStorage.getItem('2file-uploads')
    if (uploads) {
  ...
  methods: {
    forget(i) {
      ...
      localStorage.setItem('2file-uploads', JSON.stringify(this.uploads))
    },
    clearAll() {
      this.uploads = []
      localStorage.removeItem('2file-uploads')
    },
  },
}
</script>
